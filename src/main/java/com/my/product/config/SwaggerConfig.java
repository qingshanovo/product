...
public class SwaggerConfig {
    ...
    public Docket createRestApi() {
        return new Docket(DocumentationType.SWAGGER_2).groupName("spring-test-interface")
                ...
                .select()
                //加载swagger 扫描包
                .apis(RequestHandlerSelectors.basePackage("com.my.build.controller"))
    ...
    private ApiInfo apiInfo() {
        Contact contact = new Contact("tom", "", "tom@cat.com");
        return new ApiInfoBuilder().title("swagger测试").description("API在线文档").contact(contact)
                .version("1.0.0").build();
    }
